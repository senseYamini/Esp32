...
#define IPERF_SERVER_DURATION 600
...
void iperf_server_init() {
    ...
    server_addr.sin_port = htons(IPERF_SERVER_PORT);
    ...
    ESP_LOGI(TAG, "iperf2 server started");

    // Set timeout for the server to run for the desired duration
    vTaskDelay(pdMS_TO_TICKS(IPERF_SERVER_DURATION * 1000));

    // Server has run for the desired duration, stop the server
    ESP_LOGI(TAG, "Stopping iperf2 server");
    close(server_socket);
}
